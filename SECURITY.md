...
- **Once minted it is scoped to the user account, not to a device or browser.** Any client that has it can replay it from anywhere. That property is the only reason this plugin can authenticate at all, and it is also what makes the cookie dangerous.
- **Minting one requires trusting the browser.** Alarm.com issues the cookie whenever two-factor verification completes, but a value from a sign-in where you did not tick *remember/trust this device* works only in the browser that created it. The durable, portable version — the dangerous one — is the one you get by trusting the browser.
- **It is long-lived.** It remains valid for an extended period with no expiry that the plugin can see or control. Your Homebridge host does not need to be a trusted device: testing confirmed the cookie is honoured from a machine that Alarm.com reports as untrusted, so what matters is the browser it came from, not the machine replaying it.
