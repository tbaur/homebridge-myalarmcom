...
const reporter_1 = require("./diagnostics/reporter");
...
class MyAlarmComPlatform {
    ...
    constructor(log, config, api) {
        ...
        this.#rawLog = log;
        const { config: resolved, warnings, errors } = (0, validators_1.validateConfig)(config);
        ...
        this.#log = (0, logger_1.createScopedLogger)(log, 'platform', this.#config.debug);
        this.#partitionLog = (0, logger_1.createScopedLogger)(log, 'partition', this.#config.debug);
        ...
        this.#diagnostics = new collector_1.DiagnosticsCollector({
            pluginVersion: version_1.PLUGIN_VERSION,
            config: this.#config,
        });
        this.#reporter = new reporter_1.DiagnosticsReporter({
            collector: this.#diagnostics,
            readers: this.#buildDiagnosticsReaders(),
            log: (0, logger_1.createScopedLogger)(log, 'diagnostics', this.#config.debug),
            intervalMs: this.#config.diagnosticsInterval * settings_1.MS_PER_SECOND,
        });
        if (!resolved) {
            this.#log.error(`${settings_1.PLATFORM_NAME} is not starting because its configuration is unusable. `
        ...
        this.#client = new client_1.AlarmComClient({
            ...
            metrics: (sample) => this.#diagnostics.apiRequest(sample.durationMs, sample.isOk, sample.wasNetworked),
        ...
        api.on('didFinishLaunching', () => {
            ...
            // rather than assumed.
            this.#start().catch((error) => this.#reportFailure('Startup failed', error));
        });
    ...
    get client() {
        if (!this.#client) {
            throw new errors_1.ConfigurationError(`${settings_1.PLATFORM_NAME} has no usable configuration`);
        }
        return this.#client;
    }
    /**
    ...
    async #awaitInitialDiscovery() {
        ...
        for (;;) {
            ...
            try {
                ...
                if (delayMs === null) {
                    this.#log.error(`Initial discovery failed and will not be retried: ${detail}. `
                        + 'Correct the problem and restart Homebridge.');
                    return false;
                }
                attempt++;
                this.#log.warn(`Initial discovery failed: ${detail}. Retrying in ${Math.round(delayMs / settings_1.MS_PER_SECOND)}s.`);
    ...
    #sleepUnlessShuttingDown(ms) {
        ...
        return new Promise((resolve) => {
            ...
            (0, retry_1.sleep)(ms, this.#abortController.signal)
    ...
    async #discover(options) {
        ...
        this.#removeStaleAccessories(new Set([...requestedPartitionIds, ...requestedSensorIds]), this.#config.ignoredDeviceIds);
        await this.#publishDevices(requestedPartitionIds, requestedSensorIds, reason, signal);
    }
    #withoutIgnored(deviceIds) {
        return deviceIds.filter((id) => !this.#config.ignoredDeviceIds.has(id));
    }
    #logInventory(devices, reason) {
        const inventory = `${devices.partitionIds.length} partition(s) and ${devices.sensorIds.length} sensor(s)`;
        if (reason === 'startup') {
     ...
     */
    async #publishDevices(partitionIds, sensorIds, reason, signal) {
        ...
        if (unreadable.length > 0) {
            this.#log.warn(`Alarm.com returned no detail for ${unreadable.length} device(s); keeping their HomeKit accessories until it does.`);
        }
        ...
        this.#removeStaleAccessories(new Set([...this.#partitions.keys(), ...this.#sensors.keys(), ...unreadable]), skipped);
    }
    ...
    #syncPartition(resource) {
        ...
        if (!handler) {
            handler = new partition_1.PartitionAccessory(this, accessory, this.#partitionLog);
    ...
    #syncSensor(resource, reason) {
        ...
        if (!kind) {
            announce(`Skipping "${displayName}" (${deviceId}): Alarm.com device type ${String(attributes.deviceType)} is not supported yet.`);
        ...
        if (attributes.isMonitoringEnabled === false && !this.#config.includeUnmonitoredSensors) {
            announce(`Skipping "${displayName}" (${deviceId}): Alarm.com reports monitoring is disabled. Set "includeUnmonitoredSensors" to expose it anyway.`);
            this.#forgetSensor(deviceId);
            return false;
        }
        ...
        if (!handler) {
            handler = new sensor_1.SensorAccessory(this, accessory, kind, this.#sensorLog(kind));
    ...
    #sensorLog(kind) {
        ...
        const created = (0, logger_1.createScopedLogger)(this.#rawLog, kind, this.#config.debug);
        this.#sensorLogs.set(kind, created);
        return created;
    }
    /** Stop tracking a sensor and cancel any timer it owns. */
    #forgetSensor(deviceId) {
     ...
     */
    #removeStaleAccessories(liveIds, skippedIds = new Set()) {
    ...
    #startPolling() {
        ...
        const intervalMs = this.#config.pollIntervalSeconds * settings_1.MS_PER_SECOND;
    ...
    #startKeepAlive(sessionManager) {
        ...
        this.#keepAliveTimer = setInterval(() => {
            sessionManager.touch()
                ...
                .catch((error) => {
                this.#log.debug(`Session keep-alive tick failed: ${(0, sanitizers_1.sanitizeError)(error)}`);
            });
    ...
    async #startEventStream() {
        ...
        this.#eventStream = new event_stream_1.EventStream({
            log: (0, logger_1.createScopedLogger)(this.#rawLog, 'events', this.#config.debug),
    ...
    requestDeviceRefresh(deviceId) {
        ...
        }, settings_1.REFRESH_DEBOUNCE_MS);
     ...
     */
    async #refreshDevices(deviceIds, signal) {
        const partitionIds = [];
        const sensorIds = [];
    ...
    async #refreshAll() {
        ...
        let didFail = false;
        try {
            devicesRefreshed = await this.#withCycleDeadline((signal) => this.#runRefreshCycle(isRediscoveryDue, signal));
    ...
    async #runRefreshCycle(isRediscoveryDue, signal) {
        if (isRediscoveryDue) {
    ...
    #reportFailure(context, error) {
        ...
        if ((0, failure_log_level_1.failureLogLevel)(error) === 'error') {
