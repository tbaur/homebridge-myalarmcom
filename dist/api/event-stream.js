...
class EventStream {
    ...
    getStatus() {
        ...
        return {
            ...
            disconnectAgeSec: isConnected || this.#disconnectedAt === null
                ? null
                : Math.round((Date.now() - this.#disconnectedAt) / settings_1.MS_PER_SECOND),
        };
    }
    ...
    async #openSocket(attempt, { token, endpoint }) {
        ...
        if (attempt.shouldDeferDispose && this.isConnected) {
            ...
            this.#connectGeneration++;
            this.#disposeCandidate();
            const candidate = new ws_1.default(url);
     ...
     */
    #awaitHandshake(socket, options) {
        const { generation, isRefreshCandidate } = options;
        return new Promise((resolve) => {
            let isSettled = false;
            const handshake = { timer: null };
            const settle = () => {
                if (isSettled) {
                    return;
                }
            ...
            socket.on('unexpected-response', (_request, response) => {
                this.#onHandshakeTransportFailure(ctx, `the server refused the connection upgrade with HTTP ${response.statusCode}`);
            });
        });
    }
    #onHandshakeTimeout(ctx, settle) {
        ...
        if (ctx.isRefreshCandidate && ctx.isCandidateSocket() && this.isConnected) {
            this.#log.debug(`refresh handshake timed out after ${settings_1.WEBSOCKET_HANDSHAKE_TIMEOUT_MS}ms; `
    ...
    #onHandshakeOpen(socket, ctx, settle) {
        if (!ctx.isCurrentGeneration()) {
            settle();
            return;
        }
    ...
    #onHandshakeClose(ctx, code, settle) {
        if (ctx.isRefreshCandidate && ctx.isCandidateSocket()) {
            this.#candidate = null;
            this.#isConnecting = false;
            if (this.isConnected && !this.#isStopped) {
                this.#log.debug(`refresh candidate closed with code ${code}; keeping the live socket`);
                this.#scheduleRefreshRetry();
            }
    ...
    #onHandshakeTransportFailure(ctx, reason) {
        if (!ctx.isCurrentGeneration()) {
            return;
        }
        if (ctx.isRefreshCandidate && ctx.isCandidateSocket() && this.isConnected) {
    ...
    #disposeSocket() {
        ...
        const wasOpen = this.#socket.readyState === ws_1.default.OPEN;
    ...
    #scheduleReconnect() {
        ...
        if (this.#consecutiveFailures > settings_1.WEBSOCKET_MAX_FAILURES) {
            ...
            const summary = 'Alarm.com event stream unavailable; falling back to polling. '
                + `Will retry in ${Math.round(settings_1.WEBSOCKET_RECOVERY_INTERVAL_MS / settings_1.MS_PER_MINUTE)} minutes.`;
