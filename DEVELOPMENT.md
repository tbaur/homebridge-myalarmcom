...
| `src/devices/partition.ts` | Security panel accessory. Presents read-only unless `allowHomeKitArming` is on and the account has `hasPermissionToChangeState: true`. |
...
- **Dev-dependency hygiene.** An `overrides` entry pins transitive `js-yaml` to `^3.15.2` (GHSA-5p4m-2wfm-xmqj and GHSA-2883-xcg3-v3hh on 3.x via Jest coverage). It is dev-only and never shipped. The minimum is floating so a later 3.x advisory fix is picked up instead of held back.
...
- **Every API line names its request.** `createScopedLogger` redacts secrets and gates debug output; it does not invent a second component prefix — Homebridge already tags lines with the plugin name. API lines carry a six-hex request tag and a duration, shared by every retry and failure belonging to the same logical request — without which a log of thousands of near-identical poll lines cannot be reasoned about at all.
...
- **Permission is checked before acting.** A partition the account may not control is refused locally, before a command is sent, rather than after it fails: HomeKit gets `INSUFFICIENT_PRIVILEGES` and the log reports that the Alarm.com account used cannot change the arming state of that partition. The check is fail-closed, so anything other than a literal `hasPermissionToChangeState: true` counts as "may not control". `allowHomeKitArming` (default on) is the same gate for installs that want a display-only tile: HomeKit has no PIN prompt, so the Home app is otherwise the keypad.
- **A slow command is not a failed one.** Alarm.com holds an arming request open until the panel acknowledges, measured at 17–25 seconds, while HAP abandons a set handler at 10. `PARTITION_COMMAND_DEADLINE_MS` therefore ends the *wait* rather than the command: HomeKit is told the request was accepted, the pending target keeps the tile on the requested state, and the outcome is logged and reconciled when it lands. Treating that deadline as a timeout reported a failure for every arm that was seconds from succeeding.
- **An arm that cannot succeed is refused before it is sent.** A panel asked to arm over an open contact does not answer at all, so the request ran to its ceiling and reported a failure a full minute after a tap the plugin could already have answered. With `allowSensorBypass` off, an open contact refuses the arm immediately and names the sensor. Only contacts count, because motion and smoke do not stop a panel arming, and the check is skipped entirely on accounts with more than one partition: Alarm.com reports sensors per system, so there is no way to attribute an open door to the partition being armed.
- **Commands get a longer ceiling than reads.** `PARTITION_COMMAND_TIMEOUT_MS` (60s) applies to arming and disarming; everything else keeps `DEFAULT_REQUEST_TIMEOUT_MS` (30s). Two disarms of one panel minutes apart took 19.4s and 25.4s, so the read ceiling left under five seconds of headroom on the slower of them. A command cut off there is reported as a failed arm while the panel carries on arming, which is the one outcome a security integration must not produce.
